...
@dataclasses.dataclass
class GlobalState():
    selected: BooleanList = dataclasses.field(default=BooleanList())
...
selected = BooleanList(Reactive([False] * len(FILE_LIST)))
GLOBAL_STATE = GlobalState(selected=selected, file_list=Reactive(FILE_LIST))


@solara.component
def ListFileItem(file: dict, idx: int):
    def on_toggle(value):
        GLOBAL_STATE.selected.toggle(idx)
    ...
    with rv.ListItem() as main:
        ...
        with rv.ListItemAction() as action:
            solara.Checkbox(value=GLOBAL_STATE.selected.get(idx), on_value=on_toggle)
